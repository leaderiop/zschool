...
English translation, 2026-09-09. Synthesis of research passes: FatouratiCMI, CardPayments, WalletChannels, WhatsAppAPI, SMSAggregators.
...
- PRD takeaway (DEC-31): the acquirer/payment-institution contract is held **by the school**; ZSchool is a technical provider (integration, tokenization on the acquirer's side, an agent that never holds funds); V2 card rails = NAPS e-Premium or Chari Pay or a bank-affiliated payment-institution acquirer.
...
- PRD takeaway: the matrix of payment methods = cash (cash-drawer session) / cheque (with a full lifecycle) / bank transfer / semi-manual direct debit / **Fatourati (creditor model, V1)** / tokenized recurring card (V2); ZSchool never holds funds.
...
- PRD takeaway (DEC-36): WhatsApp utility messaging is the primary channel (opt-in), push notifications are the first channel (free), alias SMS is the critical fallback; budget for **inbound replies too** after 01/10/2026; the pricing switch is a dated parameter.
...
- Market contract model: self-service, **prepaid by the school** (credits with no expiry); for ZSchool: resold packs (DEC-28: MAD 0.30-0.50/SMS with a margin, consistent with the alias rate) or a ZSchool-owned aggregator account with pass-through billing; no formal reseller program found published.
- PRD takeaway: channel hierarchy push → WhatsApp utility → alias SMS; LowCost reserved for non-critical mass alerts; credit counters and threshold alerts per school.
