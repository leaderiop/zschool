...
> | Change History | 1.0 (2026-09-09): Migrated from `prd/cross-cutting/39-risks-mitigations.md` (v0.3, English translation); `R-NN` renamed `RSK-ZS-NNN`; review arbitrations ADR-ZS-047, ADR-ZS-048, ADR-ZS-049, ADR-ZS-062, ADR-ZS-066, ADR-ZS-042, and the escalated founder decisions ADR-ZS-071/072/073, carried over as-is (CCR-ZS pending). 1.1 (2026-09-09): RSK-ZS-011 redefined from OCI af-casablanca-1 (single AD) to AWS `eu-central-1`/`eu-west-3` (single-region dependency) per ADR-ZS-091 (Accepted); OQ-ZS-372 marked Resolved (CCR-ZS-002). |
...
| RSK-ZS-011 | Hosting: single-region dependency on AWS `eu-central-1`, catalogue to be verified, DR plan to be proven | Technical | Medium | High | High | All versions |
...
### RSK-ZS-011 — Securing hosting: single-region dependency on AWS `eu-central-1`, catalogue to verify, DR plan to prove

| Attribute | Value |
|---|---|
| Category | Technical |
| Description | **Redefined by [ADR-ZS-091](decisions/091-eu-hosting-deviation-from-morocco-baseline.md) (Accepted, 2026-09-09)**, which moved hosting from the Oracle Cloud "Morocco West" region (af-casablanca-1) to AWS `eu-central-1` (Frankfurt) — the original single-availability-domain risk on af-casablanca-1 no longer applies, since `eu-central-1` is multi-AZ. What remains is a **single-region dependency**: a region-level AWS event would interrupt the platform if nothing is planned for. The legal basis is now the CNDP EU-adequacy list (Deliberation No. 236-2015) rather than in-country hosting (ADR-ZS-007, superseded baseline): data residency stays inside the EU/EEA rather than inside Morocco, at the accepted cost of the "hosted in Morocco" commercial argument (ADR-ZS-091 Consequences). Separately, the availability of managed services in `eu-central-1` (managed Postgres, Lambda, S3, Cognito) still needs a service-by-service check at launch (INT-ZS-039): an architecture designed around a missing service would require a mid-course redesign. |
| Probability | Medium |
| Impact | High |
| Exposure | High |
| Preventive mitigation | A service-by-service check of the `eu-central-1` catalogue at design time and at launch (INT-ZS-039), an architecture with no critical dependency on single-AZ services; daily encrypted backups with a minimum 30-day retention and quarterly tested restoration (baseline §10); **from MVP onward**, daily replication of encrypted backups to a second EU region, `eu-west-3` (Paris) (SEC-ZS-026, INT-ZS-036, the RDM-ZS-013 gate — ADR-ZS-066 (§25h), ADR-ZS-091); **at V1**, a full DR plan with failover (SEC-ZS-025, NFR-ZS-009), RTO of 8 h / RPO of 15 min targets limited to a region disaster, with the 99.5% SLA read as excluding region disasters (ADR-ZS-066 §25i, historical alias D5); semi-annual restoration exercises; the only outbound flows outside the EU/EEA remain messaging providers governed by the F118 model. |
| Fallback plan | If the primary region fails: rebuild in `eu-west-3` from replicated backups, per the announced RTO communicated to schools; a standard crisis-communication template (leadership, schools, parents via school channels); if a required managed service proves unavailable in `eu-central-1`: switch to an equivalent self-managed service or to `eu-west-3`, with no data leaving the EU/EEA; an annual architecture reassessment against the actual catalogue. |
| Owner | ZSchool technical leadership; ZSchool executive leadership (hosting and backup contracts) |
| Warning indicator | Platform availability (target 99.5% outside announced maintenance, baseline §10); observed RPO and RTO during restoration exercises (a gap > 50% against targets = an alert); the gap between the expected and actual AWS `eu-central-1` catalogue (a quarterly review, INT-ZS-039); success of the off-site restoration exercise before RDM-ZS-001. Carried by the availability and continuity indicators in `spec/metrics.md` and `spec/cross-cutting/03-non-functional-requirements.md` (NFR-ZS-009). |
| Version affected | All versions (MVP: off-site replication; V1: a full DR plan) — pilot data is real student data |
| Traceability | (→ ADR-ZS-091, ADR-ZS-007 (superseded baseline), H-18, H-06, G-09, G-21, Q-08, §9, §10; INV-ZS-037; ADR-ZS-066 (§25h), ADR-ZS-066 (§25i); SEC-ZS-025, SEC-ZS-026, INT-ZS-036, INT-ZS-039) |
| External sources | `spec/research/05-infrastructure-usage.md` §1 (the superseded OCI af-casablanca-1 research); `spec/stack.md` §3 risk 3 (the current AWS `eu-central-1`/`eu-west-3` risk framing) |
...
| OQ-ZS-372 | **Resolved — [ADR-ZS-091](decisions/091-eu-hosting-deviation-from-morocco-baseline.md), Accepted 2026-09-09**: superseded ADR-ZS-007's OCI af-casablanca-1 framing entirely — hosting moved to AWS `eu-central-1`, with `eu-west-3` as the settled cross-region backup/DR site (no further "which Moroccan backup site" question to resolve). | `spec/research/05-infrastructure-usage.md` §1 (historical); `spec/stack.md` §3 risk 3; affects RSK-ZS-011. |
