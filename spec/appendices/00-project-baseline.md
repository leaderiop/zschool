# ZSchool Project Baseline (relocated, historical anchor)
...
baseline every other document in `spec/` was built from. Its own business
rules (`RG-xx`) now have normative successors as `INV-ZS-NNN` in
[`../invariants.md`](../invariants.md); its own decisions (`DEC-xx`) now have
normative successors as `ADR-ZS-NNN` in [`../decisions/`](../decisions/) (see
the crosswalk table in
[`01-review-history.md`](01-review-history.md#crosswalk-old-baseline-ids--normative-successors)).
Nothing below this line has been edited — it is reproduced exactly as
`PROJECT.md` read at the moment of migration, including its own internal
`RG-`/`DEC-`/`H-`/`G-`/`C-`/`Q-` identifiers, which stay unrenumbered here on
purpose (non-normative history is not subject to the permanence/renumbering
rules that apply to live requirements).
