...
| 9 | `@qadi` coupled by an exact pin (`effect@4.0.0-rc.112`, no version range) | The workspace upgrades in lockstep on the (Effect RC, `@qadi` 0.4.x) pair; `@qadi` application-level authorization and Postgres RLS (risk 10, resolved) are now two independent enforcement layers |
| 10 | ~~Neon grants every project role `BYPASSRLS`...~~ **Resolved.** The `BYPASSRLS` behavior is specific to roles provisioned via `neonctl roles create` (verified against `zschool_app`: `ALTER ROLE zschool_app NOBYPASSRLS` refused with "Only roles with the CREATEROLE attribute and the ADMIN option on role may alter this role"). A role created directly via plain SQL `CREATE ROLE ... NOBYPASSRLS` does NOT get `BYPASSRLS`, and IS alterable/droppable by `neondb_owner` — verified with a throwaway role. Migration 0007 (`packages/db/src/migrations/0007_replace_app_role.ts`) replaces `zschool_app` with `zschool_service`, provisioned this way. | ADR-ZS-092's Postgres-RLS-as-defense-in-depth (`tenant_isolation` policies, `FORCE ROW LEVEL SECURITY`) is now an actually-enforced second layer, independent of `@qadi`, for every query run through `AppSqlLive`. `zschool_app` is decommissioned (grants revoked, not dropped — Neon's ownership of that role blocks `DROP ROLE` too) rather than deleted. |
